...
def saveVectorAsImage(id, vector):
	...
	for v in range(len(vector)):
		#print(vector[v])
		pixels[v] = int(min(max(0, vector[v]), 1) * 255)
...
class NeuralNetwork:
	def __init__(self, layers):
		...
		self.cost_fun = 0.000005 * tf.reduce_sum(tf.subtract(self.logits, self.outputs) * tf.subtract(self.logits, self.outputs))
		self.train_step = tf.train.GradientDescentOptimizer(0.005).minimize(self.cost_fun)
...
my_network = NeuralNetwork([image_size, 128, image_size])
for i in range(100):
	my_network.train(training_inputs, training_outputs, 100)
	print(100 + 100 * i, "iterations")
	saveVectorAsImage(i, my_network.feed(images[0]).flatten())
